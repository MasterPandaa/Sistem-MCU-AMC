<html>
    ...
    <?php $this->load->view("meja3/navbar.php") ?>
        ...
        <div clacss="section">
			<div class="container">
                <h2>Pemeriksaan Gigi dan Mulut</h2>
                <hr>
                <?php foreach($cari as $u){ 
                 $tgl_lahir = date('d-m-Y', strtotime($u->tgl_lahir));?>
                <form action="<?php echo base_url(). 'meja3/periksa_gigi'; ?>" method="post" name="form">	
                <input type="hidden" name="jenis_dokter" value="dokter_gigi">
                        <div class="row">
				...
				<div class="col-md-8 col-sm-8">
                                ...
                                <div class="col-md-12 col-sm-12">
                                    <h4 style="color: #29166f;"><b>Hasil Pemeriksaan Gigi dan Mulut</b></h4>
                                    <div class="checkbox-group">
                                        <?php
                                        $hasil_gigi_options = [
                                            'Normal', 'Karang Gigi', 'Impaksi', 'Caries',
                                            'Ganggren', 'Missing Teeth', 'Gusi tidak sehat'
                                        ];
                                        $total_options = count($hasil_gigi_options);
                                        $options_per_row = ceil($total_options / 3);
                                        
                                        $saved_hasil_gigi = explode(', ', $u->hasil_pemeriksaan_gigi);
                                        
                                        for ($i = 0; $i < $total_options; $i++) {
                                            if ($i % $options_per_row == 0) {
                                                echo '<div class="checkbox-row">';
                                            }
                                            $is_checked = in_array($hasil_gigi_options[$i], $saved_hasil_gigi) ? 'checked' : '';
                                            echo '<div class="checkbox-item">';
                                            echo '<input type="checkbox" id="' . strtolower(str_replace(' ', '_', $hasil_gigi_options[$i])) . '" name="hasil_pemeriksaan_gigi[]" value="' . $hasil_gigi_options[$i] . '" ' . $is_checked . '>';
                                            echo '<label for="' . strtolower(str_replace(' ', '_', $hasil_gigi_options[$i])) . '">' . $hasil_gigi_options[$i] . '</label>';
                                            echo '</div>';
                                ...
                                <div class="col-md-12 col-sm-12">
                                        ...
                                        <textarea class="form-control" id="kesimpulan_gigi" name="kesimpulan_gigi" rows="3" placeholder="Masukkan Kesimpulan"><?php echo $u->kesimpulan_gigi; ?></textarea>
                                </div>
                                <div class="col-md-12 col-sm-12">
                                        <h4 style="color: #29166f;"><b>Saran</b></h4>
                                        <textarea class="form-control" id="saran_gigi" name="saran_gigi" rows="3" placeholder="Masukkan Saran"><?php echo $u->saran_gigi; ?></textarea>
                                </div>
                                <div class="col-md-12 col-sm-12">
                                        <h4 style="color: #29166f;"><b>Dokter Pemeriksa</b></h4>
                                        <select name="dr_gigi" class="form-control">
                                                <option value="">--</option>
                                                <option value="Anindita Purnama Wulandari, DRG" <?php echo ($u->dr_gigi == 'Anindita Purnama Wulandari, DRG') ? 'selected' : ''; ?>>Anindita Purnama Wulandari, DRG</option>
                                        </select>
                                        <br>
                                </div>
                                <br><br><br><br>
